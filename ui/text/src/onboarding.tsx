...
const ProviderSelector = React.memo(function ProviderSelector({ providers, height, onSelect }: ProviderSelectorProps) {
  ...
  const cardWidth = 24;  // Width of each provider card
  const cardHeight = 6;  // Height of each provider card
  const minSpacing = 2;  // Minimum spacing between cards
  
  const availableWidth = columns - 4; // Leave margins
  const availableHeight = height - 8; // Leave space for header and footer
  
  const cardsPerRow = Math.max(1, Math.floor(availableWidth / (cardWidth + minSpacing)));
  const actualSpacing = Math.floor((availableWidth - (cardsPerRow * cardWidth)) / Math.max(1, cardsPerRow - 1));
  const rowsVisible = Math.max(1, Math.floor(availableHeight / (cardHeight + 1)));
  ...
  for (let row = scrollRow; row < Math.min(scrollRow + rowsVisible, totalRows); row++) {
    ...
    if (rowProviders.length > 0) {
      visibleRows.push(
        <Box key={row} gap={actualSpacing} marginBottom={1}>
          {rowProviders}
        </Box>
      );
    }
...
const SuccessScreen = React.memo(function SuccessScreen({ provider, height, onComplete }: SuccessScreenProps) {
  ...
  // Calculate content height for proper centering (like SplashScreen)
  const contentHeight = 1 + (provider ? 1 : 0) + 2; // success message + provider text + spacing for instructions
  const topPad = Math.max(0, Math.floor((height - contentHeight) / 2));
